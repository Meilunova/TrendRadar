...
def render_html_content(
    ...
    """渲染HTML内容 - 仿NewsNow暗黑卡片风格"""
    
    # 辅助函数：生成排名徽章HTML
    def _get_rank_badge(ranks, threshold=10):
        if not ranks:
            return ""
        min_r = min(ranks)
        rank_cls = "rank-top" if min_r <= 3 else ("rank-high" if min_r <= threshold else "rank-normal")
        rank_text = str(min_r) if min(ranks) == max(ranks) else f"{min(ranks)}-{max(ranks)}"
        return f'<span class="rank-badge {rank_cls}">{rank_text}</span>'

    # 辅助函数：生成平台标签HTML
    def _get_platform_tag(source_name):
        # 简单的哈希算法来生成固定的颜色，或者根据名称指定颜色
        safe_name = html_escape(source_name)
        return f'<span class="platform-tag">{safe_name}</span>'
    ...
    <head>
        ...
        <style>
            ...
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .rank-top { background-color: var(--rank-top); }
            .rank-high { background-color: var(--rank-high); }
            .rank-normal { background-color: var(--rank-normal); color: #ccc; }
    ...
    <body>
        <div id="capture-area">
            <div class="header">
                <h1>TrendRadar 📡 热点监控</h1>
                <div class="stats-bar">
                    <div class="stat-item">模式: <strong>"""
    
    # 模式显示
    mode_text = "实时分析"
    if is_daily_summary:
        if mode == "current": mode_text = "当前榜单"
        elif mode == "incremental": mode_text = "增量模式"
        else: mode_text = "当日汇总"
    
    now = get_beijing_time()
    hot_news_count = sum(len(stat["titles"]) for stat in report_data["stats"])
    
    html += f"{mode_text}</strong></div>"
    html += f'<div class="stat-item">总数: <strong>{total_titles}</strong></div>'
    html += f'<div class="stat-item">热点: <strong>{hot_news_count}</strong></div>'
    html += f'<div class="stat-item">更新: <strong>{now.strftime("%m-%d %H:%M")}</strong></div>'
    html += """
                </div>
                <div class="action-bar" data-html2canvas-ignore>
                    <button class="btn" onclick="saveAsImage()">📸 保存为图片</button>
                    <a href="https://github.com/sansan0/TrendRadar" target="_blank"><button class="btn">GitHub</button></a>
                </div>
            </div>
    """
    ...
    if report_data["failed_ids"]:
        failed_str = ", ".join([html_escape(fid) for fid in report_data["failed_ids"]])
        html += f'<div class="error-banner">⚠️ 以下平台数据获取失败: {failed_str}</div>'
    ...
    for i, stat in enumerate(report_data["stats"]):
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="card-badge">{count} 条</div>
            </div>
            <ul class="news-list">
        """
        ...
        for title_data in stat["titles"]:
            ...
            count_n = title_data.get("count", 1)
            time_str = title_data.get("time_display", "").replace(" ~ ", "~").replace("[", "").replace("]", "")

            # 链接处理
            link_html = f'<a href="{url}" target="_blank" class="news-link">{title}</a>' if url else f'<span class="news-link">{title}</span>'
            
            # 附加信息
            meta_html = f'{_get_platform_tag(source)}'
            if time_str:
                meta_html += f'<span class="time-tag">{time_str}</span>'
            if count_n > 1:
                meta_html += f'<span class="count-tag">{count_n}次</span>'

            html += f"""
                <li class="news-item">
                    {_get_rank_badge(ranks, threshold)}
    ...
    # 3. 渲染新增新闻 (New Titles) - 如果有的话
    if report_data["new_titles"]:
        html += f"""
        <div class="section-divider">
            <span>🆕 本次新增热点 ({report_data['total_new_count']} 条)</span>
        </div>
        """
        ...
        for source_data in report_data["new_titles"]:
            ...
            <div class="card" style="border-color: #d97706;">
                <div class="card-header">
                    <div class="card-title" style="color: #fbbf24;">{source_name}</div>
                    <div class="card-badge" style="background:#78350f; color:#fbbf24;">+{titles_count}</div>
                </div>
                <ul class="news-list">
            """
    ...
    version_info = ""
    if update_info:
        version_info = f"<br><span style='color:#f59e0b'>发现新版本 {update_info['remote_version']}</span>"

    html += f"""
        <div class="footer">
            Powered by <a href="https://github.com/sansan0/TrendRadar">TrendRadar</a>
            {version_info}
        </div>
        ...
        <script>
            function saveAsImage() {{
                ...
                html2canvas(document.getElementById('capture-area'), {{
                    ...
                    const now = new Date();
                    const filename = `TrendRadar_${{now.getFullYear()}}${{now.getMonth()+1}}${{now.getDate()}}.png`;
